...
/** 从 FastAPI ResponseModel 中提取实际数据 */
function extractData(payload: any): any {
...
export const dataProvider: DataProvider = {
  getList: async (resource, params) => {
    ...
    // 安全提取：如果 payload 为 null / 非对象 / 提取后非数组 → 使用 Mock 回退
    let inner: any = null;
    let total = 0;

    if (payload && typeof payload === 'object') {
      inner = extractData(payload);
  ...
  getOne: async (resource, params) => {
    const path = apiPath(resource);
    const payload = await safeApiGet(`/${path}/${params.id}`);
  ...
  create: async (resource, params) => {
    ...
    try {
      ...
      return { data: (extractData(payload) ?? payload) as any };
    } catch {
  ...
  getMany: async (resource, params) => {
    ...
    const resolved = results.filter(Boolean).map((r) => extractData(r)).filter(Boolean);
